...
export default function CreateListing() {
    ...
    const handleImageSubmit = () =>{
        if(files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            setimageUploadError('You need to upload minimum 1 and maximum 6 images per listing');
            setUploading(false);
        }
    }
    ...
    const handleSubmit = async (e) => {
        ...
        if(+ formData.regularPrice < + formData.discountPrice) return setError('Discount price must lower then regular price');
        try {
            setLoading(true)
            setError(false)
                ...
                <p className='text-red-700 text-sm'> { imageUploadError && imageUploadError } </p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index)=>(
                        <div key={url} className="flex justify-between p-3 border items-center">
                            ...
                            <button 
                                type='button' 
                                className='p-3 text-red-700 hover:opacity-80 rounded-lg'
